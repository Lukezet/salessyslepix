          ...
          {visible.map((company) => {
            ...
            return <article className="admin-card group overflow-hidden rounded-2xl transition duration-200 hover:-translate-y-1 hover:border-amber-300/40" key={company.id}>
              ...
              <div className="space-y-4 p-5">
                ...
                <div className="grid grid-cols-3 gap-2"><button type="button" className="admin-primary px-2 py-2.5 text-sm" onClick={() => setPreview(company)}>Previsualizar</button><a href={`/${company.slug}/home`} target="_blank" rel="noreferrer" className="admin-secondary px-2 py-2.5 text-center text-sm">Acceder ↗</a><button type="button" className="admin-secondary px-2 py-2.5 text-sm" onClick={() => setManaging(company)}>Administrar</button></div>
              </div>
            </article>;
          })}
        </div>
...
function VisitorPreview({ company, onClose }) {
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label={`Previsualizacion de ${company.name}`}>
    <div className="h-[90vh] w-full max-w-[1500px] overflow-hidden rounded-2xl border border-white/15 bg-white shadow-2xl">
      ...
      <iframe title={`Portal publico de ${company.name}`} src={`/${company.slug}/home?preview=1`} className="h-[calc(100%-61px)] w-full border-0" />
    </div>
  </div>;
}
