      ...
      {loading ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
          {Array.from({ length: 8 }).map((_, i) => (
            <div
              key={i}
              className="relative h-36 md:h-44 rounded-lg overflow-hidden"
            >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-6">
          {items.map((p) => {
            ...
            return (
              <Link
                key={p.id}
                to={tenantPath(`/product/${p.id}`)}
                className="block inputRan rounded-xl p-3 hover:shadow"
              >
